...
public:
  SVGMarkerPosition(const SVGMarkerElement *element, const Point &origin,
                    float angle)
...
public:
  ...
  bool isRenderable() const {
    return !m_path.isNull() && !isDisplayNone() && !isVisibilityHidden();
  }
  ...
  void updateMarkerPositions(SVGMarkerPositionList &positions,
                             const SVGLayoutState &state);
  void render(SVGRenderState &state) const override;

  const Path &path() const { return m_path; }
  const SVGPaintServer &fillPaintServer() const { return m_fill; }
  const SVGPaintServer &strokePaintServer() const { return m_stroke; }
